...
//       // Calculate total self-business and total team business
//       calculateBusinessTotals(response?.data);
...
//   const calculateBusinessTotals = (data) => {
...
//     totalTeamBusiness: 0
//   });
//   const [paginatedTotals, setPaginatedTotals] = useState({
//     totalSelfBusiness: 0,
//     totalTeamBusiness: 0
//   });
...
//       setGenerationTeam(response?.data);
//       setTotalPages(response?.totalPages || 1);
//       // Calculate total self-business and total team business for the current page
//       calculatePaginatedTotals(response?.data);
//     } catch (error) {
//       BasicInfo.isDebug && console.error("Error fetching payment transactions:", error);
//     } finally {
//       setLoading(false);
...
//     try {
//       const queryParams = {
//         limit: 100000, // Or some large number to ensure all data is fetched
//         ...Object.fromEntries(
//           Object.entries(search).filter(
//             ([_, value]) => value !== "" && value !== 0
//           )
...
//         `${ApiPaths.getALlTeam}?${new URLSearchParams(queryParams).toString()}`
//       );
//       calculateAllDataTotals(response?.data);
//     } catch (error) {
//       BasicInfo.isDebug && console.error("Error fetching all data totals:", error);
//     }
//   };

//   const calculatePaginatedTotals = (data) => {
//     const totalSelfBusiness = data.reduce((sum, member) => sum + (member?.self_investment || 0), 0);
//     const totalTeamBusiness = data.reduce((sum, member) => sum + (member?.business || 0), 0);
//     setPaginatedTotals({ totalSelfBusiness, totalTeamBusiness });
//   };

//   const calculateAllDataTotals = (data) => {
//     const totalSelfBusiness = data.reduce((sum, member) => sum + (member?.self_investment || 0), 0);
//     const totalTeamBusiness = data.reduce((sum, member) => sum + (member?.business || 0), 0);
//     setAllDataTotals({ totalSelfBusiness, totalTeamBusiness });
//   };
...
//               )}
//               <h4>
//                 Self Business (Current Page): <span>{paginatedTotals.totalSelfBusiness || 0} {companyData?.currency}</span>
//               </h4>
//               <h4>
//                 Team Business (Current Page): <span>{paginatedTotals.totalTeamBusiness || 0} {companyData?.currency}</span>
//               </h4>
//             </div>
//           </div>
